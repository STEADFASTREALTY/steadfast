...
function SiteShell({ site, listings, media, assets, testimonials }: { site: NonNullable<Awaited<ReturnType<typeof getProfessionalSite>>>; listings: SiteListing[]; media: SiteMedia[]; assets: {id:string;placement:string}[]; testimonials: Testimonial[] }) {
  ...
  const style = { "--site-primary": palette.primary, "--site-accent": palette.accent, "--site-background": palette.background, "--site-text": palette.text, "--site-on-primary": heroTextColor, "--site-on-primary-muted": heroSecondaryTextColor, "--agent-site-background-image": heroBackgroundAsset ? `url("/media/sites/${heroBackgroundAsset.id}/display.webp?v=${heroBackgroundAsset.id}")` : undefined, backgroundColor: palette.background, color: palette.text } as CSSProperties;
  const section = {
    hero: site.site_type === "agent"
      ? <section key="hero" className="professional-site-hero agent-site-hero"><div className="agent-site-ocean-banner" aria-hidden="true" /><div className="agent-site-profile-panel">{asset ? <Image className="site-profile-photo" src={`/media/listings/${asset.id}/card.webp?v=${asset.id}`} alt={`${site.display_name} profile photograph`} width={420} height={420} unoptimized /> : null}<div className="professional-site-hero-copy"><AutoFitHeading className="brokerage-site-title"><HeroColorText color={palette.text}>{site.display_name}</HeroColorText></AutoFitHeading><p><HeroColorText color={palette.text}>{site.headline ?? "Local property guidance with clear, professional service."}</HeroColorText></p></div></div></section>
