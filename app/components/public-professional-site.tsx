...
function AgentHeroName({ name, color }: { name: string; color: string }) {
  const nameParts = name.trim().split(/\s+/);
  const lastName = nameParts.pop() ?? "";
  const firstNames = nameParts.join(" ");
  return <><span style={{ color }}>{firstNames}{firstNames ? " " : ""}</span><span className="agent-hero-last-name">{lastName}</span></>;
}
...
function SiteShell({ site, listings, media, assets, testimonials }: { site: NonNullable<Awaited<ReturnType<typeof getProfessionalSite>>>; listings: SiteListing[]; media: SiteMedia[]; assets: {id:string;placement:string}[]; testimonials: Testimonial[] }) {
  ...
  const section = {
    hero: site.site_type === "agent"
      ? <section key="hero" className="professional-site-hero agent-site-hero"><div className="agent-site-ocean-banner" aria-hidden="true">{heroBackgroundAsset ? <Image className="agent-site-ocean-image" src={`/media/sites/${heroBackgroundAsset.id}/display.webp?v=${heroBackgroundAsset.id}`} alt="" width={2400} height={800} unoptimized /> : null}</div><div className="agent-site-profile-panel">{asset ? <Image className="site-profile-photo" src={`/media/listings/${asset.id}/card.webp?v=${asset.id}`} alt={`${site.display_name} profile photograph`} width={420} height={420} unoptimized /> : null}<div className="professional-site-hero-copy"><AutoFitHeading className="brokerage-site-title"><AgentHeroName name={site.display_name} color={palette.text} /></AutoFitHeading><p><HeroColorText color={palette.text}>{site.headline ?? "Local property guidance with clear, professional service."}</HeroColorText></p></div></div></section>
