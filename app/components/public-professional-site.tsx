...
function SiteShell({ site, listings, media, assets, testimonials }: { site: NonNullable<Awaited<ReturnType<typeof getProfessionalSite>>>; listings: SiteListing[]; media: SiteMedia[]; assets: {id:string;placement:string}[]; testimonials: Testimonial[] }) {
  ...
  const style = { "--site-primary": palette.primary, "--site-accent": palette.accent, "--site-background": palette.background, "--site-text": palette.text, backgroundColor: palette.background, color: palette.text } as CSSProperties;
  ...
  const section = {
    hero: <section key="hero" className={`professional-site-hero ${site.site_type === "brokerage" ? "brokerage-site-hero" : "agent-site-hero"}`}><div className="professional-site-hero-surface" aria-hidden="true" style={{ backgroundColor: palette.primary }} /><div className="professional-site-hero-content">{asset ? <Image className={site.site_type === "brokerage" ? "site-logo" : "site-profile-photo"} src={`/media/listings/${asset.id}/card.webp?v=${asset.id}`} alt={`${site.display_name} ${site.site_type === "brokerage" ? "logo" : "profile photograph"}`} width={220} height={220} unoptimized /> : null}<div className="professional-site-hero-copy">{site.site_type === "brokerage" ? <AutoFitHeading className="brokerage-site-title">{site.display_name}</AutoFitHeading> : <h1>{site.display_name}</h1>}<p>{site.headline ?? (site.site_type === "agent" ? "Local property guidance with clear, professional service." : "Verified property opportunities across Jamaica.")}</p>{site.site_type === "brokerage" ? <Link className="site-join-button" href={`/join/${site.slug}`}>Join this brokerage</Link> : null}</div></div></section>,
    ...
    search: <section key="search" className="professional-site-search"><span>Find your place</span><h2>{site.site_type === "brokerage" ? `${site.display_name} properties` : `${site.display_name}'s properties`}</h2><PropertySearchCard hiddenFields={site.site_type === "brokerage" ? { brokerage: site.slug } : { agent: site.slug }} /></section>,
  ...
  return <main className="professional-site-page" style={style}>
    <style>{`.professional-site-page { background: ${palette.background} !important; color: ${palette.text} !important; }.professional-site-page .professional-site-hero { background: ${palette.primary} !important; }`}</style>
