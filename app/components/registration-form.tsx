  ...
  return (
    <form action={registerAction} className="stack-form two-column">
      ...
      {role !== "consumer" && <>
        ...
        {role === "agent" && agentMode === "brokerage" ? <label className="full"><span>Brokerage that referred you</span><select name="brokerageId" required defaultValue=""><option value="">Select your brokerage</option>{brokerages.map((brokerage) => <option value={brokerage.id} key={brokerage.id}>{brokerage.display_name}</option>)}</select></label> : role === "broker" ? <label className="full"><span>Brokerage name</span><input name="brokerageName" minLength={2} maxLength={160} required placeholder="Your brokerage company name" /><small>Brokerage names are verified as unique across ProperAP, including organizations awaiting review.</small></label> : null}
