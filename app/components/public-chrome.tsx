...
export function PublicHeader() { return <header className="public-header"><BrandLogo /><nav aria-label="Public navigation">{links.map(([label, href]) => <Link key={href} href={href}>{label}</Link>)}</nav><PublicCurrencySelector /><Link className="outline-button" href="/sign-in">Sign in</Link></header>; }
