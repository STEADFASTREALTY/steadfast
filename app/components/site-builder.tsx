...
export function SiteBuilder({ site, testimonials, assets }: { site: Site; testimonials: Testimonial[]; assets: SiteAsset[] }) {
  ...
  return <section className="account-card site-builder-card">
    ...
    <form id={`site-builder-save-${site.id}`} action={saveSiteBuilderAction} className="stack-form" data-prompt-title="Publish these website changes?" data-prompt-message="Your professional website layout, palette, and public content will update for visitors." data-prompt-confirm="Save website">
      ...
      <fieldset className="theme-editor"><legend>Your color palette</legend><p className="theme-editor-help">Choose colors for the visible page details. The hero uses the banner image and cannot be changed here. Changes stay in this form until you save the website.</p>{themeFields.map(({ key, label, help }) => <label key={key}><span>{label}</span><input type="color" name={`theme-${key}`} value={siteTheme[key]} onChange={(event) => setSiteTheme((current) => ({ ...current, [key]: event.target.value }))} /><small>{help}</small></label>)}<input type="hidden" name="theme" value={JSON.stringify(siteTheme)} readOnly /></fieldset>
      <label><span>About section headline</span><input maxLength={160} value={aboutHeading} onChange={(event) => setAboutHeading(event.currentTarget.value)} placeholder={site.site_type === "brokerage" ? "For example: A brokerage portfolio in one clear place." : "For example: Service built around your property goals."} /><small>This appears as the heading at the start of your About section.</small></label>
