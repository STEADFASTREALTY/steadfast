...
export function CurrencyPrice({ priceJmd, pricePeriod, rates }: { priceJmd: number; pricePeriod: string | null; rates: ExchangeRateSnapshot | null }) {
  ...
  return <div className="currency-price">
    <div className="currency-price-value"><span>JMD</span><strong>{formatAmount(priceJmd, "JMD")}{pricePeriod ? <small> / {pricePeriod}</small> : null}</strong><small>Official asking price</small></div>
    {rates ? <div className="currency-price-conversions">
      {convertedCurrencies.map((currency) => <div key={currency}><span>{currency}</span><strong>{formatAmount(convertedAmount(priceJmd, currency, rates), currency)}{pricePeriod ? <small> / {pricePeriod}</small> : null}</strong><small>Estimated</small>{currency === "GBP" ? <span className="currency-info" tabIndex={0} aria-describedby={tooltipId}>i<span id={tooltipId} role="tooltip" className="currency-disclaimer">Converted prices use rates provided by <a href="https://www.exchangerate-api.com" target="_blank" rel="noreferrer" aria-label="ExchangeRate-API">ExchangeRate</a>. Exchange rates change continuously. CanadaSAP does not guarantee that these conversions reflect current conversion rates and is not responsible for inaccuracies. Please independently verify information before relying on it. Rates updated {new Intl.DateTimeFormat("en-JM", { dateStyle: "medium", timeStyle: "short", timeZone: "America/Jamaica" }).format(new Date(rates.provider_updated_at))}.</span></span> : null}</div>)}
    </div> : null}
  </div>;
}
