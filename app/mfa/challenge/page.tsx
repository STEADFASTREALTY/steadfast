...
export const metadata: Metadata = { title: "Verify authenticator", description: "Verify your identity before opening restricted ProperAP tools.", robots: { index: false, follow: false } };
...
export default async function MfaChallengePage({ searchParams }: { searchParams: Promise<{ next?: string }> }) {
  ...
  return <main className="auth-page"><section className="auth-intro"><BrandLogo /><span className="eyebrow"><i /> Identity check</span><h1>One more<br />step.</h1><p>Enter the current code from your authenticator app to open restricted ProperAP tools.</p></section><section className="auth-card"><div><span className="eyebrow dark"><i /> Secure verification</span><h2>Verify it’s you</h2><p>Authenticator codes change every 30 seconds.</p></div><MfaChallenge nextPath={next} /></section></main>;
}
