...
export default async function MfaChallengePage({ searchParams }: { searchParams: Promise<{ next?: string }> }) {
  ...
  return <main className="auth-page"><section className="auth-intro"><Link className="brand" href="/"><span className="brand-mark">S</span><span>SteadFast</span></Link><span className="eyebrow"><i /> Identity check</span><h1>One more<br />step.</h1><p>Enter the current code from your authenticator app to open restricted SteadFast tools.</p></section><section className="auth-card"><div><span className="eyebrow dark"><i /> Secure verification</span><h2>Verify it’s you</h2><p>Authenticator codes change every 30 seconds.</p></div><MfaChallenge nextPath={next} /></section></main>;
}
