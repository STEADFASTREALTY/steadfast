...
export const metadata: Metadata = { title: "Set up authenticator", description: "Protect restricted ProperAP work with an authenticator.", robots: { index: false, follow: false } };
...
export default async function MfaSetupPage({ searchParams }: { searchParams: Promise<{ next?: string }> }) {
  ...
  return <main className="auth-page"><section className="auth-intro"><BrandLogo /><span className="eyebrow"><i /> Two-step security</span><h1>Protect your<br />access.</h1><p>ProperAP internal work requires a password and a current authenticator code.</p></section><section className="auth-card"><div><span className="eyebrow dark"><i /> Required setup</span><h2>Add an authenticator</h2><p>Scan one secure setup code, then confirm that your app is generating valid six-digit codes.</p></div><MfaEnrollment nextPath={next} /><p className="auth-switch">Your password remains unchanged. <Link href="/account">Return to account</Link></p></section></main>;
}
