...
export default async function SubscriptionPage({ searchParams }: { searchParams: Promise<{ error?: string; notice?: string }> }) {
  ...
  const isRegisteredUser = !context.membership && !registration;
  return <main className="account-page"><AccountHeader displayName={context.person.display_name} hasWorkspace={access.hasWorkspace} canManageAgents={access.canManageAgents} canManageListings={access.isAgent || access.canReviewListings} canManageInquiries={access.canManageInquiries} canShareListings={access.canShareListings} isConsumer={!context.membership} />
    ...
    <div className="account-settings-layout account-subscription-layout">{!context.membership ? <ConsumerAccountNav active="subscription" /> : <AccountSectionNav active="subscription" />}<div className="account-main"><StatusMessage error={query.error} notice={query.notice} /><section className="account-card subscription-card"><div className="card-heading"><span>Current plan</span><h2>{isRegisteredUser ? "Registered User" : subscription ? planName(subscription.plan_key) : "Professional registration"}</h2></div><dl><div><dt>Subscription date</dt><dd>{isRegisteredUser ? formatDate(context.person.created_at) : formatDate(subscription?.starts_at ?? registration?.created_at ?? null)}</dd></div><div><dt>End date</dt><dd>{isRegisteredUser ? "Free account" : formatDate(subscription?.ends_at ?? null)}</dd></div><div><dt>Plan type</dt><dd>{isRegisteredUser ? "Registered User · Free" : subscription ? `${subscription.billing_period === "none" ? "Free access" : subscription.billing_period} · ${subscription.status}` : `${registration?.request_type === "agent" ? "Agent" : "Broker"} · ${registration?.status.replaceAll("_", " ")}`}</dd></div></dl></section>{isRegisteredUser ? <section className="account-card"><div className="card-heading"><span>Professional access</span><h2>Upgrade to Agent or Broker</h2></div><p>Send your request to ProperAP for review. Your Registered User account remains free while we review it.</p><ProfessionalUpgradeForm brokerages={brokeragesResult.data ?? []} /></section> : null}{!context.membership && registration && !isRegisteredUser ? <section className="account-card"><div className="card-heading"><span>Review status</span><h2>Your professional request is being reviewed</h2></div><p>ProperAP will notify you when your {registration.request_type} access is approved and activated.</p></section> : null}{isBroker && plan ? <section className="account-card subscription-capacity"><div className="card-heading"><span>Brokerage capacity</span><h2>Available team slots</h2></div><div><article><span>Agents</span><strong>{plan.agentLimit === null ? "Unlimited" : Math.max(plan.agentLimit - agentCount, 0)}</strong><small>{agentCount} of {plan.agentLimit ?? "Unlimited"} currently used</small></article><article><span>Staff</span><strong>{plan.staffLimit === null ? "Unlimited" : Math.max(plan.staffLimit - staffCount, 0)}</strong><small>{staffCount} of {plan.staffLimit ?? "Unlimited"} currently used</small></article></div></section> : null}</div></div>
  </main>;
}
