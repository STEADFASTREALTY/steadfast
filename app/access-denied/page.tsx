...
export default async function AccessDeniedPage({ searchParams }: { searchParams: Promise<{ reason?: string }> }) {
  ...
  return <main className="account-page"><AccountHeader displayName={context.person.display_name} hasWorkspace={access.hasWorkspace} canManageAgents={access.canManageAgents} /><section className="denied-card"><span>Access unavailable</span><h1>{message.title}</h1><p>{message.detail}</p><div><Link className="solid-button" href={access.hasWorkspace ? "/workspace" : "/account"}>Return to your workspace</Link><Link className="outline-dark-button" href="/properties">Browse properties</Link></div></section></main>;
}
