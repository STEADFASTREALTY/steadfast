...
| TM-007 — Inquiry, registration or search abuse | Availability, inboxes and costs; spam, scraping and degradation | Anonymous automation; high | Inquiry payload bounds, honeypot, idempotency and database email/property quotas | Add edge/provider traffic limits and bot signals, monitor per-source patterns, and retain database limits as defense in depth; residual medium. |
...
| TM-010 — Public projection, cache or log leakage | Personal data, drafts and exact address; high privacy/regulatory harm | Wrong DTO, cache or logging; medium | Allowlisted listing projections, private inquiry RLS, safe notification/outbox summaries, dynamic private routes and sitemap rules | Maintain DTO and negative-permission tests, log redaction, private response controls, and schema-diff canaries; residual low/medium. |
